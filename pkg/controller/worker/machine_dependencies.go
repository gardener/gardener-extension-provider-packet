...
import (
	...
	util "github.com/gardener/gardener-extension-provider-packet/pkg/util"
...
const (
	vpnSeedConfigMap                      = "equinix-vpn-seed-config"
	vpnSeed                               = "vpn-seed"
	apiServerDeploy                       = "kube-apiserver"
	nodeNetwork                           = "nodeNetwork"
	nodeNetworkEnvVar                     = "NODE_NETWORK"
	equinixMetalPrivateNetworkAnnotations = "metal.equinix.com/private-network-4"
...
func (w *workerDelegate) CleanupMachineDependencies(ctx context.Context) error {
	...
	for _, n := range shootNodes.Items {
		if n.Annotations == nil || n.Annotations[equinixMetalPrivateNetworkAnnotations] == "" {
			...
			fmt.Printf("deviceID %s err %#v", deviceID, err)
			if deviceID == "" || err != nil {
				continue
			}
			nodePrivateNetwork, err := getNodePrivateNetwork(deviceID, c, ns)
			if err != nil {
...
func (w *workerDelegate) UpdateConfigMap(ctx context.Context) error {
	ns := w.worker.GetNamespace()
	c := w.Client()

	// get the private IPs from the shoot nodes
	_, shoot, err := util.NewClientForShoot(ctx, c, ns, client.Options{})
	if err != nil {
		return err
	}
	...
	cm := &corev1.ConfigMap{}
	if err := c.Get(ctx, client.ObjectKey{
		Namespace: w.cluster.ObjectMeta.GetNamespace(),
		Name:      vpnSeedConfigMap,
	}, cm); err != nil {
		return fmt.Errorf("failed to get configmap: %v", err)
	}

	// we have the configmap, update it with the right CIDR for nodes
	existing, ok := cm.Data[nodeNetwork]
	// if it already exists, check if we are updating it
	if ok {
		// check for a conflict
		if existing != cidrs {
			return fmt.Errorf("conflicting cidr, existing %s replacement %s", existing, cidrs)
		}
		// matched, so nothing left to do
		return nil
	}
	cm.Data[nodeNetwork] = cidrs
...
func getNodePrivateNetwork(deviceID string, kClient client.Client, namespace string) (string, error) {
